...
# -*- coding: UTF-8 -*-
'''
@Project ：yolov5-animal 
@File    ：window_animal.py
@Author  ：ChenmingSong
@Date    ：2022/3/9 20:37 
@Description：
'''
...
class MainWindow(QTabWidget):
    ...
    def __init__(self):
        # 初始化界面
        super().__init__()
        self.setWindowTitle('毕设系列-检测专题-动物检测系统')
        self.resize(1200, 800)
        self.setWindowIcon(QIcon("images/UI/xxx.jpg"))
        # 图片读取进程
        ...
        if source == "":
            ...
            for path, im, im0s, vid_cap, s in dataset:
                ...
                for i, det in enumerate(pred):  # per image
                    ...
                    class_names = ['bird', 'cat', 'cow', 'dog', 'horse', 'sheep']
                    class_nums = [0 for i in range(len(class_names))]
                    if len(det):
                        ...
                        for *xyxy, conf, cls in reversed(det):
                            class_nums[int(cls)] = class_nums[int(cls)] + 1
                    ...
                    for name, num in zip(class_names, class_nums):
                        if num != 0:
                            record = record + "{}: {}".format(name, num)
                    # print(record)
                    cv2.putText(im0, record, (20, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
    ...
    def detect_vid(self):
        ...
        for path, im, im0s, vid_cap, s in dataset:
            ...
            for i, det in enumerate(pred):  # per image
                ...
                cv2.putText(frame_resized, record, (20, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
